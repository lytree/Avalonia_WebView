...
internal class Interop_gdk
{
    ...
    public static ulong g_signal_connect(
        nint instance,
        string detailed_signal,
        nint c_handler,
        nint data
    ...
    [DllImport(_Gdk)]
    public static extern nint g_memory_input_stream_new_from_data(
        byte[] data,
        uint len,
        nint destroy
    );
    ...
    public static void SetWindowParent(nint gdkWindow, nint x11Window) =>
        gdk_window_set_transient_for(gdkWindow, x11Window);

    public static nint GetForeignWindow(nint display, nint xid) =>
